...
segmentIntersect 
  (Segment _ (V2 hx1 hy1) (V2 hx2 _)) 
  (Segment _ (V2 vx1 vy1) (V2 _ vy2))
      | hy1 `member` vys && vx1 `member` hxs = Just (V2 vx1 hy1)
      | otherwise = Nothing
      where 
...
intersections s1 s2 = filter (/= V2 0 0) $ catMaybes $ toList $ (segmentIntersect <$> hs2 <*> vs1) >< (segmentIntersect <$> hs1 <*> vs2)
  where
...
part1 = fmap (minimum . map (manhatten (V2 0 0)) . uncurry intersections . bimap runPath runPath) <$> input

pointIntersect :: V2 Integer -> Segment -> Bool
pointIntersect (V2 x y) (Segment Horiz (V2 sx sy) (V2 endx _)) = y == sy && x `member` mkInterval sx endx 
pointIntersect (V2 x y) (Segment Vert (V2 sx sy) (V2 _ endy)) = x == sx && y `member` mkInterval sy endy 

distToIntersection :: V2 Integer -> Seq Segment -> Integer
distToIntersection i p = beforeSum + afterSum
  where
    beforeSum =  sum $ fmap (\(Segment{start, end}) -> manhatten start end) before
...
part2 = do
  ...
  let is = intersections p1 p2 <&> \i -> distToIntersection i p1 + distToIntersection i p2
  pure $ minimum is
