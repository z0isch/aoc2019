...
input :: IO Prog
input = V.fromList . map (fromJust . readMaybe) . sepBy ',' <$> readFile "data/day2.txt"
...
runProg noun verb p = V.head $ last $ go (Just (p V.// [(1,noun),(2,verb)],0))
    where
        go Nothing      = []
...
part2 :: IO Int
part2 = ((\(n,v,_) -> n*100+v) . head . dropWhile (\(_,_,s) -> s /= 19690720) . allCombinations) <$> input
